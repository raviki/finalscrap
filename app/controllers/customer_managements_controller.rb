class CustomerManagementsController < ApplicationController
  ...
  def create
    ...
    if !(customer_management_params[:email].present? || customer_management_params[:mobile_number].present?)
      ...
      if @customer_management.save
        @customer_management.send_welcome_message
  ...
  private
    ...
    def customer_management_params
      params.require(:customer_management).permit(:name,:role, :password, :mobile_number, :password_confirmation, :email, :customer_id, :remember_token, :password_digest, :provider, :uid, :oauth_token, :oauth_expires_at, :password_reset_token, :password_reset_sent_at)
    end
end
